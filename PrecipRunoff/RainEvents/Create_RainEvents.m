...
%   clear thisEvent
  ...
  % Add the Soil Moisture values.
  [SMStartIdx, SMEndIdx] = findTimeIdx(thisEvent.startTime, thisEvent.endTime,...
                               SMData.(sites{1}).TIME, numLeading, numTrailing);
...
for i=1:length(startTimes)
  ...
  % Add the Soil Moisture values.
  [SMStartIdx, SMEndIdx] = findTimeIdx(thisEvent.startTime, thisEvent.endTime,...
                               SMData.(sites{2}).TIME, numLeading, numTrailing);
  % Add just the indexed portion of each field in the SM struct.
...
function [startIdx, endIdx] = findTimeIdx(precipStart, precipEnd, ...
                                          dataTimes, numBefore, numAfter)
    % If precip end occurs before the dataTimes even begin default to returning a single timestamp.
    % TODO: decide if this is the best "error code".
    if precipEnd <  dataTimes(1)
        startIdx = 1;
        endIdx = 1;
        return
    end

    % TODO: Might want to restrict the SM time so that it doesn't extend too far past the precip.
    % Unlike all other data, the SM data is in 15 minute resolution. So just adding "numBefore" extra points will actually add numBefore*5 more minutes than expected.
    % timePerSample = dataTimes(2) - dataTimes(1);
    % if (timePerSample) ~= duration(0, 10,0)
    %     % Modify numBefore and numAfter
    % end

    startIdx = max(find(dataTimes > precipStart ,1) - numBefore, 1);
    endIdx = min(find(dataTimes > precipEnd ,1) + numAfter, length(dataTimes));
end
